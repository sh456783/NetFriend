...
그리고 **온프레미스 물리 서버(VM)** 에서 수집한 데이터를 웹 UI로 확인할 수 있도록 구성된 대시보드입니다.  

Suricata IDS 로그, CloudWatch 메트릭(EC2 + 물리 서버), EC2 제어 기능과 함께  
**IP 차단 목록(자동/수동)을 한 화면에서 관리**할 수 있습니다.
...
### ● IDS 로그 모니터링
- Suricata가 생성한 `eve.json` 이벤트를 FastAPI에서 수집/가공  
- 탐지 시간, 시그니처, 출발지/목적지 IP 등 기본 탐지 정보 표시  
- 위험도(Severity)에 따라 알림을 구분하여 표시
